...
function W(a) {
    ...
    return (s.searchParams.set(M, e), { cacheKey: s.href, url: n.href });
}
class A {
    constructor() {
        ((this.updatedURLs = []),
...
async function j(a, e) {
    let t = null;
    if ((a.url && (t = new URL(a.url).origin), t !== self.location.origin)) throw new l('cross-origin-copy-response', { origin: t });
    const s = a.clone(),
...
class V {
    ...
    async fetch(e) {
        ...
        try {
            for (const c of this.iterateCallbacks('requestWillFetch')) s = await c({ request: s.clone(), event: t });
        } catch (c) {
            if (c instanceof Error) throw new l('plugin-error-request-will-fetch', { thrownErrorMessage: c.message });
        }
        const r = s.clone();
        try {
            let c;
            c = await fetch(s, s.mode === 'navigate' ? void 0 : this._strategy.fetchOptions);
            for (const i of this.iterateCallbacks('fetchDidSucceed')) c = await i({ event: t, request: r, response: c });
            return c;
        } catch (c) {
            throw (n && (await this.runCallbacks('fetchDidFail', { error: c, event: t, originalRequest: n.clone(), request: r.clone() })), c);
        }
    }
    ...
    async cacheMatch(e) {
        ...
        for (const o of this.iterateCallbacks('cachedResponseWillBeUsed'))
            s = (await o({ cacheName: n, matchOptions: r, cachedResponse: s, request: c, event: this.event })) || void 0;
        return s;
    }
    async cachePut(e, t) {
        const s = m(e);
        ...
        if (!t) throw new l('cache-put-with-no-response', { url: F(n.url) });
        ...
        const { cacheName: c, matchOptions: i } = this._strategy,
            ...
            g = h ? await H(o, n.clone(), ['__WB_REVISION__'], i) : null;
        try {
            await o.put(n, h ? r.clone() : r);
        } catch (u) {
            if (u instanceof Error) throw (u.name === 'QuotaExceededError' && (await G()), u);
        }
        for (const u of this.iterateCallbacks('cacheDidUpdate'))
            await u({ cacheName: c, oldResponse: g, newResponse: r.clone(), request: n, event: this.event });
        return !0;
    }
...
class T {
    ...
    async _getResponse(e, t, s) {
        ...
        let n;
        try {
            if (((n = await this._handle(t, e)), !n || n.type === 'error')) throw new l('no-response', { url: t.url });
        } catch (r) {
            if (r instanceof Error) {
                for (const c of e.iterateCallbacks('handlerDidError')) if (((n = await c({ error: r, event: s, request: t })), n)) break;
            }
            if (!n) throw r;
        }
    ...
    async _awaitComplete(e, t, s, n) {
        ...
        if ((await t.runCallbacks('handlerDidComplete', { event: n, request: s, response: r, error: c }), t.destroy(), c)) throw c;
    }
}
class d extends T {
    constructor(e = {}) {
...
class Q {
    constructor({ cacheName: e, plugins: t = [], fallbackToNetwork: s = !0 } = {}) {
        ((this._urlsToCacheKeys = new Map()),
            ...
            (this._strategy = new d({ cacheName: C.getPrecacheName(e), plugins: [...t, new D({ precacheController: this })], fallbackToNetwork: s })),
    ...
    addToCacheList(e) {
        ...
        for (const s of e) {
            ...
            const { cacheKey: n, url: r } = W(s),
                c = typeof s != 'string' && s.revision ? 'reload' : 'default';
            if (this._urlsToCacheKeys.has(r) && this._urlsToCacheKeys.get(r) !== n)
                throw new l('add-to-cache-list-conflicting-entries', { firstEntry: this._urlsToCacheKeys.get(r), secondEntry: n });
    ...
    install(e) {
        return L(e, async () => {
            ...
            for (const [r, c] of this._urlsToCacheKeys) {
                ...
                await Promise.all(this.strategy.handleAll({ params: { cacheKey: c }, request: h, event: e }));
            }
...
class J {
    ...
    handleRequest({ request: e, event: t }) {
        ...
        const n = s.origin === location.origin,
            { params: r, route: c } = this.findMatchingRoute({ event: t, request: e, sameOrigin: n, url: s });
        let i = c && c.handler;
        const o = e.method;
    ...
    findMatchingRoute({ url: e, sameOrigin: t, request: s, event: n }) {
        ...
        for (const c of r) {
            ...
            if (o)
                return (
                    (i = o),
                    ((Array.isArray(i) && i.length === 0) || (o.constructor === Object && Object.keys(o).length === 0) || typeof o == 'boolean') &&
                        (i = void 0),
                    { route: c, params: i }
                );
        }
...
function E(a, e, t) {
    ...
    else if (typeof a == 'function') s = new p(a, e, t);
    else if (a instanceof p) s = a;
...
function* Y(
    a,
    { ignoreURLParametersMatching: e = [/^utm_/, /^fbclid$/], directoryIndex: t = 'index.html', cleanURLs: s = !0, urlManipulation: n } = {},
) {
    const r = new URL(a, location.href);
    ((r.hash = ''), yield r.href);
    const c = X(r, e);
...
class ce extends p {
    ...
    _match({ url: e, request: t }) {
        ...
        for (const n of this._denylist) if (n.test(s)) return !1;
...
class oe extends T {
    ...
    async _handle(e, t) {
        ...
        try {
            ...
            if (((n = await Promise.race(r)), !n)) throw new Error(`Timed out the network response after ${this._networkTimeoutSeconds} seconds.`);
        } catch (r) {
            r instanceof Error && (s = r);
        }
        if (!n) throw new l('no-response', { url: e.url, error: s });
        return n;
    }
}
self.skipWaiting();
ae();
re(self.__WB_MANIFEST);
const le = '/offline.html';
E(new ce(new oe(), { denylist: [/^\/api\//] }));
ie(async ({ event: a }) => (a.request.destination === 'document' && (await caches.match(le))) || Response.error());
