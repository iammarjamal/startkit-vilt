...
- small: show as small heading (with title/description)
- title: heading text (for small)
- description: description text (for small)
...
<template>
    <header v-if="small">
        <h3 class="mb-0.5 text-base font-medium text-[var(--foreground)]">{{ title }}</h3>
        <p v-if="description" class="text-sm text-[var(--muted-foreground)]">{{ description }}</p>
    </header>
    <h1 v-else :class="['font-bold text-[var(--foreground)]', sizeClass, props.class]">
        <slot />
    </h1>
</template>
