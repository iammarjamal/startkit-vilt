...
<template>
    <App :title="props.title" :desc="props.desc">
        <div class="scrollbar-none flex h-full max-h-screen mb-16 md:mb-0 min-h-screen w-full flex-row overflow-hidden bg-background text-foreground">
            <!-- Desktop Sidebar -->
            <Transition
                ...
                >
                    <nav class="flex h-full w-full flex-col items-center justify-center">
                        ...
                        <div class="mb-2 flex w-full flex-row items-center justify-between gap-2 overflow-x-hidden md:justify-center">
                            <div class="mx-1 flex w-full flex-col items-center justify-center transition-all duration-300" :class="!isSidebarOpen ? 'mt-5 mb-3' : 'mt-0.5 mb-0'">
            ...
            <Drawer v-if="!isDesktop" v-model:open="isSidebarOpen">
                <DrawerContent class="h-full w-full bg-background border-0 rounded-none">
                    <div class="flex h-full min-h-96 flex-col p-4 mt-6">
                        <!-- Navigation -->
                        <div class="flex-1 overflow-y-scroll scrollbar-none">
            ...
            <!-- Main Content -->
            <div
                class="flex-1 flex flex-col bg-background text-foreground transition-all duration-300 ease-out overflow-hidden"
                :class="[
                    'xs:border-none sm:border xs:border-none md:m-4 md:ms-0 md:me-0 md:rounded-2xl md:rounded-e-none md:border',
                    isDesktop ? (isSidebarOpen ? 'md:ml-0' : 'md:ml-0') : ''
                ]"
            >
                <!-- Header -->
                <header class="flex-shrink-0 flex h-10 items-center border-b border-border py-8 dark:border-border" v-if="isDesktop">
                    <div class="flex w-full items-center gap-4 px-4 md:px-6">
                        <button
                            @click="toggleSidebar()"
                            class="inline-flex cursor-pointer items-center justify-center rounded-md text-sm font-medium whitespace-nowrap transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 p-2"
                ...
                <!-- Body -->
                <div class="flex-1 scrollbar-none min-w-0 overflow-y-auto overflow-x-hidden p-4 pb-24 sm:pb-0 lg:p-6">
