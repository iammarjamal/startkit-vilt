...
const { coords, error: geoError, isLoading: geoLoading, getPosition } = useGeolocation();
...
const handlePage = async (val) => {
    await Browser.open({ url: val, presentationStyle: 'popover' });
};

const handleAuth = async (val) => {
    const url = `${val}?platform=${platform.value}`;
    ...
    if (platform.value === 'android' || platform.value === 'ios') {
        ...
        console.log('[OAuth] Redirecting browser directly');
        window.location.replace(url);
    }
};

// OAuth listener reference
const oauthListener = ref(null);
onMounted(() => {
    oauthListener.value = App.addListener('appUrlOpen', async (event) => {
        try {
            if (!event.url) {
                console.warn('[OAuth] appUrlOpen fired but event.url is undefined');
                return;
            }
            ...
            if (!token) {
                console.warn('[OAuth] No token found in deep link');
                return;
            }

            console.log('[OAuth] Token received:', token);

            // Send the token in the POST body and pass callbacks as options
            router.post(route('auth.token.callback'), { token }, {
                async: true,
                onSuccess: async () => {
                    await new Promise(r => setTimeout(r, 1500));
                    router.visit(route('dashboard.index'), { method: 'get' });
                }
            });
        } catch (err) {
            console.error('[OAuth] Error handling deep link:', err);
        }
    });
});
onUnmounted(() => {
    if (oauthListener.value) {
        oauthListener.value.remove();
        oauthListener.value = null;
        console.log('[OAuth] Listener removed on component unmount');
    }
});
...
const cardVariants = (index) => ({
    ...
    animate: {
        ...
        scale: 1,
        transition: {
            delay: Math.min(0.4 + index * 0.1, 0.3),
            duration: 0.4,
...
const buttonVariants = (index) => ({
    ...
    animate: {
        ...
        scale: 1,
        transition: {
            delay: Math.min(0.4 + index * 0.15, 0.3),
            duration: 0.3,
            ease: [0.25, 0.1, 0.25, 1],
        },
    },
...
<template>
    <AuthLayout :title="t('welcome')" :description="t('hero_subtitle')"
        keywords="authentication, login, Rqeim, platform" :canGoBack="false">
            ...
            <Card class="border-none shadow-none bg-no">
                <CardHeader class="flex flex-col justify-center items-center text-center space-y-4 pb-2">
                    <div
                        ...
                        <AppLogo type="icon" :theme="isDark ? 'dark' : 'light'" />
                    </div>
                ...
                <CardContent class="space-y-6">
                    ...
                    <motion.div :initial="buttonVariants(1).initial" :animate="buttonVariants(1).animate"
                        ...
                        <Button @click="() => handleAuth(route('auth.google'))" variant="outline"
                    ...
                    <motion.div :initial="buttonVariants(2).initial" :animate="buttonVariants(2).animate"
                        ...
                        <Button @click="() => handleAuth(route('auth.microsoft'))" variant="outline"
                            class="w-full flex-row rtl:flex-row-reverse h-12 text-base font-semibold border-2 hover:bg-secondary/50 transition-all duration-300"
                            size="lg">
                    ...
                    <!-- Geolocation Button (Tauri) -->
                    <motion.div :initial="buttonVariants(3).initial" :animate="buttonVariants(3).animate"
                        :whileHover="buttonVariants(3).hover" :whilePress="buttonVariants(3).press">
                        <Button @click="getPosition" :disabled="geoLoading" variant="outline"
                            class="w-full flex-row rtl:flex-row-reverse h-12 text-base font-semibold border-2 hover:bg-secondary/50 transition-all duration-300"
                            size="lg">
                            <Icon v-if="geoLoading" icon="solar:refresh-bold" class="w-5 h-5 me-2 animate-spin" />
                            <Icon v-else icon="solar:map-point-bold" class="w-5 h-5 me-2" />
                            {{ geoLoading ? t('loading_location') : t('get_current_location') }}
                        </Button>
                        <div v-if="coords" class="mt-2 text-center text-xs text-muted-foreground">
                            {{ coords.latitude.toFixed(4) }}, {{ coords.longitude.toFixed(4) }}
                        </div>
                        <div v-if="geoError" class="mt-2 text-center text-xs text-destructive">
                            {{ geoError }}
                        </div>
                    </motion.div>
                    ...
                    <motion.div :initial="{ opacity: 0 }" :animate="{ opacity: 1 }"
                        ...
                        <div class="flex justify-center space-x-6 text-sm">
                            <button @click="() => handlePage('https://rqeim.com/page/tos')" type="button"
                                class="text-muted-foreground cursor-pointer hover:text-primary transition-colors duration-200 font-medium">
                                {{ t('terms') }}
                            </button>
                            <button @click="() => handlePage('https://rqeim.com/page/privacy')" type="button"
                                class="text-muted-foreground cursor-pointer hover:text-primary transition-colors duration-200 font-medium">
                                {{ t('privacy') }}
                            </button>
                        </div>
